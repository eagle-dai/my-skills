...
description: Use when mirroring a whole SSG-prerendered documentation site (typically built from a git repo — VitePress best-supported, Docusaurus/MkDocs via --selector) into a local markdown corpus for AI querying, RAG, or offline reading — crawls the site (sitemap/BFS), converts many pages to clean text, drops images/formulas fidelity. NOT for high-fidelity archival of SingleFile-saved HTML (use html-to-markdown for that, even in batch).
---
