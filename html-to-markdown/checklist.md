...
闭合 `**`/`*`/`_` 右侧紧跟非空白字符时，GitHub 判该定界符右-flanking 失败、配对断裂 → 星号字面显示、加粗失效（本地/VS Code 宽容，GitHub 严格）。

1. 扫描 md（先剥离 fenced code）：正则 `(\*\*[^*\n]+?\*\*)(\S)`（`\S` 覆盖 ASCII+CJK+全角标点），命中 > 0 → 阻断。同理查单 `*`/`_`。
2. 修法二选一：
   - 闭合 `**` 前是**标点**（尤其全角冒号 `：`）→ **把标点移出加粗**：`**入口核对：**按表` → `**入口核对**：按表`（最自然，标点本不该加黑）。
   - 其它情况 → **闭合定界符后插空格**：`**标签：**https://` → `**标签：** https://`。
3. 顺手查 `****` 四连星拼接 bug（加黑段相邻拼接）：去掉整行 `**` 后重包一层。
4. 详见 conversion-rules.md「加粗/斜体」段。**主 agent 必须独立复验此项，不采信 sub agent 自报。**
