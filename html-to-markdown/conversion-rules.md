...
**强调定界符闭合后紧贴非空白 → 加空格/移标点（GitHub 目标平台）：** 闭合 `**`/`*`/`_` 右侧紧跟非空白——**英文/数字/`<` 或中文汉字/全角标点都会触发**（`**标签：**https://` 和 `**概念辨析：**为什么…` 都失败），GitHub 把该定界符判为"想开新强调"（右 flanking 失败），配对失败 → 星号字面显示、加粗失效。本地/VS Code 宽容，GitHub 严格。检测 `(\*\*[^*\n]+?\*\*)(\S)`（`\S` 覆盖 ASCII+CJK，比只查 `[A-Za-z0-9<]` 全）。两种修法：
- 闭合 `**` 前是**标点**（尤其全角冒号 `：`）→ **把标点移出**：`**概念辨析：**为什么` → `**概念辨析**：为什么`（最自然，标点本不该加黑）。
- 其它情况 → **闭合定界符后插空格**：`**标签：**https://` → `**标签：** https://`。

同族坑=markdown 特殊符与内容边界冲突（另见公式 `$` 边界、数学块裸 `*`）。实测：`**专栏配套代码：**https://`（英文紧贴）、`**概念辨析：**为什么`（中文紧贴）。
