...
1. **定位**：水印是站点 logo，各图位置一致。检测方法——在图的角落区域（如右下 x>0.80、y>0.82）搜 logo 的特征色（品牌色，常为高饱和橙/红/蓝的连通块），取其 bbox 作为水印锚点。**不要硬编码坐标**，每站点先在 1-2 张图上量出规律再批量。
2. **清除**：
   - 水印压在**纯色/浅色背景**（采样周边像素方差小）→ 盖背景色矩形（从 logo 左上外扩 pad 到该角落边缘）。最快、无依赖。
   - 水印压在**内容**上 → 需 inpaint（opencv `cv2.inpaint`）或人工，盖矩形会遮内容。
3. **注意**：不是所有图都有水印（内容图、UI 截图、图表常无）——先逐图检测特征色 bbox，无命中的跳过，别误盖。
4. **验证**：去后抽查角落截图，确认水印消失、背景融合、未误盖正文。
