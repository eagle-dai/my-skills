...
| `<table>` | 外层 `data-slate-type="table"` 容器（内含 simplebar 滚动层）→ 内部为**标准 `<table><thead><tr><td>`**（带 `slate-element="table"`） | 定位用外层 `data-slate-type="table"`，抽取行列用内部标准 `<table>` 标签。Phase 1 探测须同时数 `[data-slate-type="table"]` 和 `<table>`（实测：Slate table 内部是标准标签，但探测清单里漏列 table 项本身，导致整类被忽略） |
...
- **公式编号别当公式**：原文常把公式编号（如 `D-6`、`(1)`）本身写成行内公式，提取后成 `$D - 6$`，GitHub 渲染成斜体减法式 `D − 6`。检测紧邻块级公式的孤立编号行 `^\$([A-Z])\s*-\s*(\d+)\$$` → 改**正体文本标签** `**（D-6）**`，并入公式块的 `<div align="center">` 一起居中。实测：导读 22 个 `$D - N$`。
...
部分站点在正文图**固定角落**（多为右下角）嵌站点 logo+文字水印。离线阅读包默认去水印——**但去水印是高风险操作（易误擦正文），下面每条护栏都是已踩坑，必须逐条遵守；宁可漏掉一个模糊水印，也不能盖到水印外的内容。** 只在用户明确要求"保留水印/不动图"时跳过。

**先探测再动手**：逐图在角落搜品牌特征色，无命中的图（内容图、UI 截图、纯图表常无水印）直接跳过，不误盖。检测到才走下面流程：
...
正文图片默认压缩——离线阅读包体积优先，可接受轻微有损（q80 无肉眼可见 artifact）。**只在用户明确要求"最高保真/不压缩/保留原图"时跳过。**

- **格式**：统一转 **webp**（quality 80）体积最小，PNG/JPEG 皆可转。webp 同画质比 jpeg 小 25-35%，转后与其余图格式统一。
- **限宽**：宽 > 阈值（默认 1600px，密集数字图表可保守到 2000）的等比缩到阈值，小图不动。1600px 对流程图/UI 截图完全可读。
- **同步引用**：改扩展名（如 `.jpg`→`.webp`）后，**必须同步更新 Markdown 里的图片引用路径**，否则断链。
- **工具**：机器常无 `cwebp`/PIL，`ffmpeg` 通常有——`ffmpeg -y -i in.jpg -c:v libwebp -quality 80 out.webp`。转前先探测 `which cwebp ffmpeg` + PIL。
- **验证**：转后抽检一张（Playwright 开图截图看），确认文字锐利、无块状 artifact；`ffprobe` 确认尺寸符合预期。原图体积异常大的（如 >500KB）优先确认压后无膨胀。
...
**强调定界符闭合后紧贴字符 → 加空格/移标点（GitHub 目标平台）：** 闭合 `**`/`*`/`_` 右侧紧跟**字母/数字/`<`/CJK 汉字**（`**标签：**https://` 和 `**概念辨析：**为什么…` 都失败），GitHub 把该定界符判为"想开新强调"（右 flanking 失败），配对失败 → 星号字面显示、加粗失效。本地/VS Code 宽容，GitHub 严格。

**⚠️ 标点不触发，别误伤：** CommonMark 里闭合 `**` 后紧跟 **punctuation**（含 CJK 标点 `。，、；：！？）】」』`、ASCII `.,;:!?)`）仍是**有效** right-flanking delimiter，`**结论**。` GitHub **渲染正常，不是违规**。检测正则**必须排除标点**：`(\*\*[^*\n]+?\*\*)([^\s。，、；：！？）】」』.,;:!?)])`。**别用 `(\*\*[^*\n]+?\*\*)(\S)`**——`\S` 把 CJK 标点也当违规=假阳性，会诱导对正确的句末加粗 `**结论**。` 插空格（过度修复=新 bug，2026-07-23 sub agent 实测误插 11 处 `**结论** 。`，可见多余间距）。

两种修法（**仅对真违规**，紧贴标点的不动）：
- 闭合 `**` 前是**标点**（尤其全角冒号 `：`）→ **把标点移出**：`**概念辨析：**为什么` → `**概念辨析**：为什么`（最自然，标点本不该加黑）。
- 其它情况 → **闭合定界符后插空格**：`**标签：**https://` → `**标签：** https://`。

**反向清理已误插的空格：** `\*\*[^*\n]+?\*\* [。，、；：！？）]` 命中 > 0 → 删空格。

同族坑=markdown 特殊符与内容边界冲突（另见公式 `$` 边界、数学块裸 `*`），但**方向要分清**：紧贴字符=真违规要修，紧贴标点=正确别动。实测违规：`**专栏配套代码：**https://`（英文紧贴）、`**概念辨析：**为什么`（中文汉字紧贴）；实测正确（勿修）：`**第一层读法是价格读法**。`（CJK 句号紧贴）。
