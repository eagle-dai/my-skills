...
- **单行 `<div align="center">` 内的加粗题注用 `<strong>` 不用 `**`**：GitHub 对 same-line 裸 HTML 块内的 `**bold**` 不做 Markdown 解析，星号会当字面量显示、加粗丢失。`markdown_postprocess.py::_center_caption_line` 命中单行题注时把 `**X**` 转成 `<strong>X</strong>` 再包 div（幂等，已 `<strong>` 或无加粗不动），并会修复旧产物里已包 div 但内部残留 `**` 的坏形态。上面表格/公式那种 `<div>` 换行空开再写 `**表 X-Y　标题**` 的**多行**块级形态不受影响——空行分隔时 GitHub 会当 Markdown 解析，`**` 正常渲染。
