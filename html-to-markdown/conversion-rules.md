...
| 题注 | image/figure/table 内的题注节点（`figcaption` OR Slate image 子 `<div>` OR `<caption>`） | N_caption |
...
| 图片 | `image` | 提取内部 img **+ 同级题注子节点**（见下「题注提取」） |
...
富文本编辑器常把题注（figcaption）作为**内容块的子节点或紧邻同级节点**，而非独立段落。提取内容块（图片/表格/公式）时必须一并抽出题注，否则题注静默丢失。

覆盖形态：

| 形态 | 题注位置 |
|------|----------|
| Slate `image` block | `data-slate-type="image"` 容器内，`<img>` 的**同级 `<div>`** |
| 标准 figure | `<figure>` 内的 `<figcaption>` |
| 表格 | `<table>` 内的 `<caption>`，或表格容器的同级题注节点 |

规则：

- 抽内容块时遍历其**子节点和紧邻同级节点**找题注，不能只取 `<img>`/`<table>` 本身
- 题注文本形如 `图 4-5　权益峰值...`、`表 4-3　...`（命名性、通常一行、常含全角空格 U+3000）
- 短题注按「块级居中」节与内容块同组；解释性长说明段落仍按正文左对齐，不重复当题注抽
- 题注是内容，丢失即语义损失——见验收计数要求
