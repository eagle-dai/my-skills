...
一个段落只含**多个相邻行内公式且无正文文字**时，若逐个按 `$...$` 输出会产生定界符碰撞：`$D_t=P_t/P_0-1$` 紧跟 `$T_t=D_t≤-S$` 拼成 `...-1$$T_t...`，中间的 `$$` 被 GitHub/KaTeX 当成块公式定界符，渲染破坏。

规则：

- 段落判定为**纯公式段**（无 prose）时，把其中每个行内公式**各自提升为独立 display 块**（`$$ ... $$` 单占一段，段间空行隔开）。公式总数守恒，不算丢失、不阻断。
- 判定 prose 是否为空时，**必须先排除公式子树**再取文本：Slate/KaTeX 的 `get_text()` 会把渲染字形（如 `Et=Vt∣qtPt∣`）混进来，直接判会误以为"有文字"。做法是移除 `[data-slate-type*="katex"], .katex` 子树后再看剩余文本是否为空白。
- 段内**有真实 prose**（如"设 $q_t$ 为持仓数量"）时，公式保持行内 `$...$`，不提升——只有纯公式段才提升。单个行内公式独占段落也不需提升（无相邻碰撞）。
