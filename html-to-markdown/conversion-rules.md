...
- **单行 `<div align="center">` 内的加粗/行内代码用 HTML tag 不用 Markdown 标记**：GitHub 对 same-line 裸 HTML 块内的 `**bold**` 和 `` `code` `` 都不做 Markdown 解析，会当字面量显示、格式丢失。`markdown_postprocess.py::_center_caption_line` 命中单行题注时：`**X**` → `<strong>X</strong>`（可多段），`` `Y` `` → `<code>Y</code>`；转换后仍无 `<strong>` 的裸题注（图题），把 `<code>` 之外的纯文本段各自包 `<strong>`。混排 `**A**`code`**B**`（如 `**代码 8-2　业务源码：**`normalize_candle`**…**`）也能正确拆段。幂等，已 `<strong>`/`<code>` 不重复包。会修复旧产物里已包 div 但内部残留 `**`/`` ` `` 的坏形态。上面表格/公式那种 `<div>` 换行空开再写 `**表 X-Y　标题**` 的**多行**块级形态不受影响——空行分隔时 GitHub 会当 Markdown 解析，`**` 正常渲染。
...
- **边界（重要）**：正文首段（讲本讲内容的）、思考题、正文性质的结语（有信息量的总结）都保留。寒暄若与实质内容**混在同一句**（如 `好，我们的导读篇就到这里。最后强调一下：<知识总结…>`），机械化易误删，交给人工只删寒暄引子保留后面总结。

**仓库已有实现**：`html-to-markdown/markdown_postprocess.py` 的 `_strip_author_greetings()` 做**保守机械化**（宁漏勿误删），只处理**首个/末个非空行**、删的是整行：

- **逐行判定**（不是逐块）：单 `\n` 分段（SingleFile/Slate 常见）时寒暄与正文可能同块，逐行才不会把同块正文一起删；先把 `\r\n`/`\r` 归一化成 `\n`。
- **opener 逗号必需**（`你好，我是`）：排除反问句「你好我是谁？…」；且以 `？/?` 收尾的行一律不当开场白。
- **closer 行尾锚定**（强道别短语出现在**行尾** `$`，非子串命中）：正文里出现「敬请期待」「转发」但句子没以道别收尾不会误删。裸「点赞/收藏/转发/敬请期待」正文太常见，**不作为独立信号**（否则「新版本开发中，敬请期待。」这类正文会被误删）。
