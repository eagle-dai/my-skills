...
**例外——表格 / 图片 / 图注默认居中**（排版惯例，即使原文无居中证据也居中）：

- **表标题 + 表格必须包进同一个 `<div align="center">`**，不能只居中标题。GFM 里 markdown 表格默认左对齐；`<div align="center">` 包住整表能让**表格也居中**（实测有效）。若只单独居中标题、表格不包，会出现"标题居中悬在左对齐表格上方"的不协调。标题在表上或表下都要和表格同包：
...
- **表格与标题之间必须留一个空行**（GFM 表格后无空行会把紧跟的标题行吞进表格最后一行——实测踩过）。标题在表下方时同理：表格→空行→标题，都在 div 内。
- **图片**：`![](...)` 单独包 `<div align="center">`。
- **图注 vs 正文——不能只看"图 X-Y"开头**：只有**紧跟图片、简短（一行/一两句）的图题**才居中。以 `图 X-Y` 开头但**是多句长段落、讲解制图逻辑/背景**的，是**正文**，**不居中**（居中长段落难看且语义错位）。判据：短（≤ 约 40 字）且紧邻图片 → 图题，居中；长段落 / 与图片隔着其它内容 → 正文，保持左对齐。实测：`图 D-1 给出本讲核心知识地图。它不是策略流程图……制图依据来自本讲对全专栏结构的整理。`（200+ 字）是正文，不居中。
