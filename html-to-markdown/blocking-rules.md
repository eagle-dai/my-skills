...
**强调定界符闭合紧贴非空白阻断（目标平台含 GitHub 时）：** 闭合 `**`/`*`/`_` 右侧紧跟任意非空白——**英文/数字/`<` 或中文/全角标点都触发**（`**标签：**https://`、`**概念辨析：**为什么…`），GitHub 强调配对失败、加粗失效（本地/VS Code 宽容，会漏）。检测 `(\*\*[^*\n]+?\*\*)(\S)`（`\S` 覆盖 ASCII+CJK）命中 > 0 → 阻断。修法：闭合前是标点→移标点出去，否则插空格。详见 conversion-rules.md 加粗/斜体段。
