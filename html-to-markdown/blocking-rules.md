...
**强调定界符闭合紧贴非空白阻断（目标平台含 GitHub 时）：** 闭合 `**`/`*`/`_` 右侧紧跟英文/数字/`<`（如 `**标签：**https://...`），GitHub 强调配对失败，星号字面显示、加粗失效（本地/VS Code 宽容，会漏）。检测 `(\*\*[^*\n]+?\*\*)([A-Za-z0-9<])` 命中 > 0 → 阻断，修法闭合定界符后插空格。详见 conversion-rules.md 加粗/斜体段。
