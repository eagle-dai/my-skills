...
- 删除头像、广告和纯 UI 装饰图
- 评论说明图使用 `comment_` 前缀
- **默认执行去站点水印**；只有用户明确要求保留原始水印时才跳过
- 去水印是破坏性处理：必须保留原图副本，记录处理文件和 bbox，检测/验证用原图非缩略图，特征色命中正文只取最右下连通块，逐图原尺寸验证正文未被擦除；无法安全去除时降级保留原图
- 图片压缩和去水印均按 @conversion-rules.md 默认执行，固定顺序“原图备份 → 去水印 → 压缩”
