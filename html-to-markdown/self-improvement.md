...
6. **`<li>` 内 section 裹段落穿透**：`fast_converter.py::_li_inline_passthrough_target`——li 内容遇「块 wrapper（`section/div`，无 slate，`has_block_child`）裹单个可行内段落（`<p>`/`<div>` 无更深块子）」时穿透取内层段落的 `inline_children`；裹 table/list/pre/多段落/更深块、**或 wrapper 内块之外还有非空白裸文本**（穿透会静默丢弃）→ 维持 `self.inline`，fail-close。mdnice 把有序列表项包成 `<li><section><p>提出假设</p></section></li>`。
...
| `<li><section>散文<p>提出假设</p></section></li>` | strict_required | 反例 | wrapper 块外有裸文本，穿透会丢，fail-close（PR #77 review） |
