...
与此对称，提取器侧对 math-mode 里**字面下划线**（工程标识符 `field_coverage` 等，非结构下标）产出**双反斜杠** `\\_`（缺陷 31）：GFM 剥一层后剩 `\_`，MathJax（GitHub `$$…$$` 的实际渲染器；本地 KaTeX 只是验证代理）当字面下划线（渲染对），且反转义后 `_` 前有反斜杠、不触发 identifier-as-subscript 门。单反斜杠会被剥光成裸下标（渲染错），门会正确拦下。真数学结构下标（`msupsub`/`mfrac` vlist）不经此路径、以裸 `_{…}` 输出，不受影响。见 `self-improvement.md` 缺陷 31。
